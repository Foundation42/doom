// DOOM CLI entry point - bundled version
import path from 'path';
import readline from 'readline-sync';
import chalk from 'chalk';
import fs from 'fs';
import * as os from 'os';
import { runChatWithTools, createConsoleLogger, createStandardTools } from '../src';
...
For more details visit: https://github.com/Foundation42/doom`);
  ...
  // Extract command and parallel flag
  let command = null;
  ...
  if (command) {
    await runSingleCommand(command, parallelExecution);
  } else {
    await runInteractiveMode(parallelExecution);
  }
}

// Import the entire doom.ts file content
/**
